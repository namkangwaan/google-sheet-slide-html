...
export function initializeLessons() {
  ...
  const newSlides = [
    ...
    page('lesson-answers', 'คำใบ้และเฉลยแบบฝึก 30 ข้อ', 'ฝึก → ตรวจ → อธิบาย', 'เลือกข้อที่ทำแล้ว เปรียบเทียบทั้งผลลัพธ์และเหตุผล สูตรอื่นที่ให้ผลถูกต้องก็ใช้ได้', `
      <p class="lesson-note">ตรวจตัวเองได้ทันทีในคอลัมน์ F (Self_Check): ✓ ผลถูก, ✗ ยังไม่ตรง, ⚠ พิมพ์ตัวเลขหรือข้อความแทนสูตร คะแนนรวมอยู่ที่ <code>Assignments!F3</code> ลองแก้จนขึ้น ✓ ก่อนเปิดเฉลยด้านล่าง</p><p>พิมพ์สูตรใน <code>Assignments</code> คอลัมน์ E ตรง Task_ID ข้อมูลต้นทาง HR/Sales/Regex เริ่มแถว 5 สูตรในสไลด์บางหน้าใช้ตารางสาธิตแยกต่างหาก</p>
      ...
      <div id="practice-answer" aria-live="polite"></div><p class="lesson-note">เฉลยในเว็บเปิดดูได้เพื่อเรียนรู้ ไม่ใช่ข้อสอบที่ซ่อนคำตอบ และไม่บันทึกคะแนนหรือส่งงานให้อัตโนมัติ ถ้าต้องการงานที่ครูใช้ให้คะแนน ดู ${jump('lesson-homework', 'การบ้าน 20 ข้อ')}</p>`),
    page('lesson-homework', 'การบ้าน: ร้านค้าสามสาขา', 'การบ้าน • 100 คะแนน', 'ชุดข้อมูลใหม่ 16 คำสั่งซื้อ ไม่มีเฉลยในไฟล์และในเว็บ ครูเป็นผู้ตรวจ', `
      <div class="lesson-actions"><a class="lesson-link" href="./Google_Sheets_Homework.xlsx" download>ดาวน์โหลดไฟล์การบ้าน (.xlsx)</a></div>
      <div class="lesson-columns"><div><h3>ขั้นตอน</h3><ol><li>เปิดไฟล์ใน Google Sheets แล้วตั้งชื่อ <code>Homework_ชื่อเล่น_ห้อง</code> ตั้ง Locale เป็น Thailand</li><li>ชีต <code>Orders</code>: เติมคอลัมน์สีเขียว H (Amount), I (Amount_With_VAT อ้างอิง VAT ใน <code>Settings!B2</code>) และ J (Category จากชีต Products) แล้วลากสูตรถึงแถว 20</li><li>ชีต <code>Homework_Tasks</code>: พิมพ์สูตรในคอลัมน์ E ทั้ง 20 ข้อ ข้อละ 5 คะแนน</li><li>แชร์ลิงก์สิทธิ์ผู้ดูให้ครู</li></ol></div>
      <div><h3>ทักษะที่ใช้</h3><ul><li>อ้างอิงแบบสัมพัทธ์และแบบ <code>$</code></li><li>COUNTIFS, SUMIFS, AVERAGEIF, MAXIFS</li><li>INDEX + MATCH, VLOOKUP / XLOOKUP, IFERROR</li><li>TRIM, RIGHT และการต่อข้อความด้วย <code>&amp;</code></li><li>วันที่ด้วย DATE, IF / AND ที่ค่าขอบเขต และ FILTER หลายเงื่อนไข</li></ul></div></div>
      <p class="lesson-note">ไฟล์การบ้านไม่มีคอลัมน์คำตอบและไม่มีตัวตรวจอัตโนมัติ ใช้ข้อมูลคนละชุดกับไฟล์แบบฝึก จึงนำคำตอบจากเฉลย 30 ข้อมาใช้ไม่ได้ ก่อนส่งให้ตรวจเองว่าทุกข้อเป็นสูตร และลองเปลี่ยน Qty สักแถวว่าผลเปลี่ยนตาม แล้วคืนค่าเดิม</p>
      ${questions([['ลากสูตรคอลัมน์ I ลงมาแล้วแถวล่าง ๆ ไม่คิด VAT เกิดจากอะไร?', 'อ้างอิง VAT_Rate โดยไม่ล็อก ทำให้ตำแหน่งเลื่อนตามแถวไปช่องว่าง ใช้ <code>Settings!$B$2</code>'], ['คอลัมน์ J ขึ้น #N/A หนึ่งแถว ควรทำอย่างไร?', 'ตรวจว่ารหัสสินค้านั้นมีในตาราง Products หรือไม่ ถ้าไม่มีจริงให้ครอบด้วย IFERROR และแสดงข้อความที่อ่านเข้าใจ']])}`),
