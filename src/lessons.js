...
export function initializeLessons() {
  ...
  const newSlides = [
    ...
    page('lesson-summary', 'ฝึกสรุปยอดด้วยเงื่อนไข', 'กิจกรรมพื้นฐาน', 'ทำ Classroom!E2:E5 ให้เสร็จก่อน แล้ววางสูตรสรุปใน G2', `
      ...
      ${questions([['ถ้าอยากนับจำนวนรายการอาหาร ควร SUMIF หรือ COUNTIF?', 'COUNTIF(B2:B5,"อาหาร") ได้ 2 รายการ ไม่ใช่จำนวนชิ้นที่ขาย'], ['ผลรวมอาหารควรเท่าไร และตรวจโดยไม่ใช้สูตรอย่างไร?', '146 บาท จากน้ำ 56 + ขนม 90 ตรวจอีกทางด้วย 396 − 250']])}
    ...
    page('lesson-charts', 'จากตารางสู่กราฟที่ตอบคำถาม', 'พื้นฐาน 06', 'คำถาม: หมวดใดมียอดขายมากกว่า? ใช้กราฟแท่งเพื่อเปรียบเทียบ', `
      <ol><li>ใช้ชีตใหม่ชื่อ Summary กรอก A1 = Category, B1 = Amount; A2 = เครื่องเขียน, A3 = อาหาร</li><li>ที่ B2 พิมพ์สูตรด้านล่าง แล้วลากถึง B3</li><li>เลือก A1:B3 → Insert → Chart → เลือก Column chart ตั้งชื่อ “ยอดขายแยกตามหมวด (บาท)”</li><li>ตรวจแกนหมวดหมู่และหน่วย อ่านค่าจากกราฟเทียบตาราง</li></ol>${formula('=SUMIF(Classroom!B$2:B$5,A2,Classroom!E$2:E$5)')}
    ...
    page('lesson-advanced-capstone', 'ชิ้นงานต่อยอด: ตรวจผลอีกทาง', 'การประเมินต่อยอด', 'ใช้ตารางบนสไลด์ E-Commerce และ Warehouse พิมพ์ลงชีตใหม่ หัวแถว 1 ข้อมูลเริ่มแถว 2', `
      <ol><li>E-Commerce: เพิ่มคอลัมน์ Net Sales ใช้ Gross − Discount เฉพาะ Is_Returned = FALSE และใช้ QUERY สรุปตาม Platform</li><li>Warehouse: สกัด SKU จาก Raw_Barcode และคำนวณ Need_Restock โดยเปรียบเทียบ Unit_Stock กับ Reorder_Point</li><li>ส่งชีตข้อมูล สูตรที่ใช้ ผลสรุป และคำอธิบายว่าตรวจข้อมูลคืนสินค้า/สต็อกเท่าจุดสั่งซื้ออย่างไร</li></ol>
      ${reveal('คำใบ้', 'E-Commerce ใช้ IF ก่อน QUERY; Warehouse ใช้ REGEXEXTRACT และ IF(C2<=D2,...)')}
      ${reveal('ตรวจผลและแนวเฉลย', `${formula('=IF(F2=FALSE,D2-E2,0)')}<p>ใน E-Commerce!G2 ลากถึง G4: ได้ 2200, 8400, 0 รวม 10600; สูตรนี้กำหนดให้รายการคืนสินค้าไม่รวมในยอดขาย</p>${formula('=QUERY(C1:G4,"select C, sum(G) group by C order by sum(G) desc",1)')}<p>Warehouse: SKU-8821 / SKU-9042 และ OK / REORDER NOW ตามลำดับ</p>`)}
